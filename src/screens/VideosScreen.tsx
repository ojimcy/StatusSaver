...
import {Lock, Video, Heart} from 'lucide-react-native';
...
const VideosScreen: React.FC<{navigation: any}> = ({navigation}) => {
  ...
  const filteredVideos = showFavoritesOnly
    ? videos.filter(v => favoriteIds.includes(v.id))
    : videos;
  ...
  const renderContent = () => {
    ...
    if (showFavoritesOnly && filteredVideos.length === 0) {
      return (
        <EmptyState
